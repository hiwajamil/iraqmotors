import '../../models/localized_car_model.dart';

const List<LocalizedCarModel> renaultSamsungMotorsModels = [
  LocalizedCarModel(id: 'qm5', ku: 'QM5', en: 'QM5', ar: 'QM5'),
  LocalizedCarModel(id: 'qm6', ku: 'QM6', en: 'QM6', ar: 'QM6'),
  LocalizedCarModel(id: 'sm3', ku: 'SM3', en: 'SM3', ar: 'SM3'),
  LocalizedCarModel(id: 'sm5', ku: 'SM5', en: 'SM5', ar: 'SM5'),
  LocalizedCarModel(id: 'sm6', ku: 'SM6', en: 'SM6', ar: 'SM6'),
  LocalizedCarModel(id: 'sm7', ku: 'SM7', en: 'SM7', ar: 'SM7'),
  LocalizedCarModel(id: 'xm3', ku: 'XM3', en: 'XM3', ar: 'XM3'),
];
